...
export interface Contract {
  ...
  content: string;
  softwareIds?: string[]; // Array of software UUIDs
  contractType?: string; // Loại hợp đồng
  contractTypeId?: string; // UUID
  valuePreVat: number;
...
export type WarningType = 'acceptance_upcoming' | 'acceptance_overdue' | 'payment_upcoming' | 'payment_overdue' | 'contract_expired';
...
export interface WarningItem {
  ...
  daysDiff: number; // Positive = Remaining, Negative = Overdue
  amount: number; // Related money amount (if payment)
  pic: string; // Person In Charge
  status: 'pending' | 'processing' | 'resolved';
  note: string;
  details?: string; // Extra info like "Đợt 2 - 30%"
}